...
| Relational layer (SQL) | `com.litedb.relational` | `Catalog`, `TableSchema`, `IndexDef`, `TypeCodec`, `RelationalEngine` | ✅ DDL + DML + multiple indexes + query planner + typed encoding + **runs on MVCC transactions** |
| Atomic write batch | `com.litedb.engine` | `WriteOp`, `LSMEngine#writeBatch` | ✅ WAL BEGIN/COMMIT — all-or-nothing on recovery |
| MVCC transactions | `com.litedb.mvcc` | `MVCCEngine`, `Transaction`, `TimestampOracle` | ✅ snapshot isolation + OCC conflict detection + tombstones + GC — **wired into the SQL layer** (`BEGIN`/`COMMIT`/`ROLLBACK`) |
